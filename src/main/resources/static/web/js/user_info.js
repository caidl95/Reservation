...
function pageTo(pageSize, curPage) {
    ...
    var html = " <table id='table-users' style= 'font-size: 20px;' >" +
        ...
        "<td  style='width: 15%;padding: 5px'>用户名</td>" +
        "<td style=\"width: 14%\">昵称</td>" +
        "<td style=\"width: 14%\">角色</td>" +
        "<td style=\"width: 14%\">邮箱</td>" +
    ...
    for (var index = 0; index < datas.length; index++) {
        ...
        html += "<td style='width: 15%;padding: 5px'>#{loginname}</td>";
        html += "<td style=\"width: 14%\">#{nickname}</td>";
        html += "<td style=\"width: 14%\">#{role}</td>";
        html += "<td style=\"width: 14%\">#{email}</td>";
        html += "<td style=\"width: 14%\">#{phone}</td>";
        html += "<td style=\"width: 14%\">#{createTime}</td>";
        html += "<td style=\"width: 14%\"><input style=' border: 0px solid ;background-color: rgba(0, 0, 0, 0)' type='button' value='删除' onclick='condition_user_delete(#{id})'>&nbsp; <input onclick='condition_users_update(#{id})' style=' border: 0px solid ;background-color: rgba(0, 0, 0, 0)' type='button' value='修改'></td></tr>";

        html = html.replace(/#{id}/g, data.id);
        html = html.replace(/#{loginname}/g, data.loginname);
        html = html.replace(/#{nickname}/g, data.nickname);
...
function condition_users_update(id) {
    ...
    $("#updateNickname").attr("value", "");
    $("#updateEmail").attr("value", "");
    $("#updatePhone").attr("value", "");
    if (isNone) {

        $.ajax({
            "url": "/users/get_user_id?id="+id,
            "type": "GET",
            "dataType": "json",
            "success": function (json) {
                if (json.status == 200) {
                    ...
                    $("#updateNickname").attr("value", json.data.nickname);
